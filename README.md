# Moonlit

Moonlit is a build and release automation engine built on Rust and WebAssembly.
Pipelines are declared in YAML and executed by a `wasmtime`-based host that runs
sandboxed WebAssembly plugin components.

> **Status:** early development.

## License

Moonlit is open source, dual-licensed under either of

- [Apache License 2.0](LICENSE-APACHE)
- [MIT license](LICENSE-MIT)

at your option. You may use, copy, modify, and distribute it freely, including in
commercial and closed-source products. See [NOTICE](NOTICE) for trademark terms.

A managed cloud service is planned at moonlitbuild.cloud.

## Contributing

See [CONTRIBUTING.md](CONTRIBUTING.md).

## Trademarks

"Moonlit" and the Moonlit logo are trademarks of Wolfware LLC. See
[TRADEMARKS.md](TRADEMARKS.md) and [NOTICE](NOTICE).
